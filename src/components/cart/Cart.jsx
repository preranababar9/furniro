        ...
        <div className=" flex w-full max-md:flex-col  items-center lg:justify-around  bg-white gap-8  text-black text-sm">
          ...
          <div className=" flex flex-col justify-start gap-5">
            {cartItems.map((item, index) => (
              <div key={index} className="flex max-md:flex-col  justify-between  gap-2 lg:gap-20 items-center">
                <div className="flex  lg:gap-10  gap-4  items-center">
                  <img
                    src={item.imageUrl}
                    alt={item.title}
                    className="rounded-md lg:h-[200px] h-[100px] lg:w-full w-3/4"
                  />
                  <div className="flex flex-col w-full">
                    <h1 className="lg:text-2xl text-lg font-bold">{item.title}</h1>
                    <p className="text-gray-600 lg:text-xl text-md">{item.price}</p>
                  </div>
                </div>
          ...
          {cartItems.length > 0 ? (
            <div className="flex flex-col  ">
              <h1 className="text-2xl font-bold">Total: ${getCartTotal()}</h1>
              <button
